...
// Oeffentliche Origin der App, derselbe Wert wie die CORS-Origin: lokal der Dev-Server,
// der /api an den BFF proxyt. Ein abschliessender Schraegstrich ergaebe einen doppelten
// Slash in der redirect_uri – Keycloak vergleicht byteweise und lehnt das ab.
const APP_BASE_URL = requiredEnv('ALLOWED_ORIGIN').replace(/\/+$/, '');

// Kein offline_access: ein Offline-Token ueberlebt den SSO-Logout und haette kein
// Idle-Timeout. So entscheiden Keycloaks SSO Session Idle/Max, wann die Session endet.
